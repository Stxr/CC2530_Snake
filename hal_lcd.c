...
__code const uint8 ascii_table_4[][16];
char offset,flag=0;
static unsigned int LCD_Display_char[8][128]={0};
/**************************************************************************************************
 * @fn      HalLcd_HW_WaitUs
 *
...
void HalLcd_writedot(unsigned char x,unsigned char y,unsigned char point){
  set_ddram_line_col(x,y/4);
}
/*
...
//    logo
    if(flag==0){
      ...
      for(int i=0;i<3;i++){
        for(k=0; k<16; k++) {
          if(k%2==0){
            set_ddram_line_col(0,i*16+k/2+offset); //run only one time
            HalLcd_HW_Write(ascii_table_4[2*i][k]);
          }else{
            set_ddram_line_col(1,i*16+(k-1)/2+offset);  //run only one time
            HalLcd_HW_Write(ascii_table_4[2*i][k]);
          }
        }
        for(k=0;k<16;k++){
          if(k%2==0){
            set_ddram_line_col(0,i*16+k/2+8+offset); //run only one time
            HalLcd_HW_Write(ascii_table_4[2*i+1][k]);
          }else{
            set_ddram_line_col(1,i*16+(k-1)/2+8+offset);  //run only one time
            HalLcd_HW_Write(ascii_table_4[2*i+1][k]);
          }
        }
      }
...
参数2   pText。待显示的字符串
 **************************************************************************************************/
void HalLcd_HW_WriteLine(unsigned int line, const char *pText,unsigned char point) {

    uint8 count;
    uint8 totalLength = getStrlen( (char *)pText );
    for (count=0; count<totalLength; count++) {
        HalLcd_HW_WriteChar(line,count+point,*pText);
        pText++;
    }
...
void HalLcd_HW_Display(unsigned int a[][128]){
  uint8 i,j;
  for(i=0;i<8;i++){
    set_ddram_line_col(i,0);
    for(j=0;j<128;j++){
      HalLcd_HW_Write(a[i][j]);
    }
  }
}
...
__code const uint8 ascii_table_4[][16]= {
    ...
    {0x94,0x04,0x93,0x04,0x92,0x24,0x96,0x44,0xFA,0x24,0x02,0x1C,0x02,0x00,0x00,0x00},/*"第",0*/

    {0x00,0x00,0xFC,0x7F,0x04,0x28,0x04,0x24,0x04,0x23,0xFC,0x20,0x04,0x20,0x04,0x20},
    {0x04,0x20,0xFC,0x21,0x04,0x22,0x04,0x22,0x04,0x22,0xFC,0x7F,0x00,0x00,0x00,0x00},/*"四",1*/

    {0x20,0x22,0x30,0x67,0xAC,0x22,0x63,0x12,0x30,0x12,0x00,0x40,0x00,0x40,0xFE,0x7F},
    {0x22,0x42,0x22,0x42,0x22,0x42,0x22,0x42,0xFE,0x7F,0x00,0x40,0x00,0x40,0x00,0x00},/*"组",2*/
