...
//    logo
    if(flag==0){
      flag++;
      // offset=35;
    //  HalLcd_HW_DisplayChinese(0,0,35);
    }
}
...
void HalLcd_HW_DisplayChinese(int i,int line,int offset){
//  for(int i=num;i<3;i++){
    for(int k=0; k<16; k++) {
      if(k%2==0){
        set_ddram_line_col(line,i*16+k/2+offset); //run only one time
        HalLcd_HW_Write(ascii_table_4[2*i][k]);
      }else{
        set_ddram_line_col(line+1,i*16+(k-1)/2+offset);  //run only one time
        HalLcd_HW_Write(ascii_table_4[2*i][k]);
      }
    }
    for(int k=0;k<16;k++){
      if(k%2==0){
        set_ddram_line_col(line,i*16+k/2+8+offset); //run only one time
        HalLcd_HW_Write(ascii_table_4[2*i+1][k]);
      }else{
        set_ddram_line_col(line+1,i*16+(k-1)/2+8+offset);  //run only one time
        HalLcd_HW_Write(ascii_table_4[2*i+1][k]);
      }
    }
//  }
...
__code const uint8 ascii_table_4[][16]= {
    ...
    {0xB2,0x01,0x64,0x21,0x28,0x21,0x10,0x5F,0x10,0x40,0x20,0x80,0x20,0x00,0x00,0x00},/*"贪",3*/

    {0x00,0x00,0xFC,0x0F,0x04,0x04,0x04,0x04,0xFC,0x0F,0x20,0x00,0x10,0x30,0x4C,0x48},
    {0x4B,0x44,0x48,0x42,0x48,0x42,0x48,0x41,0xC8,0x40,0x08,0x40,0x08,0x70,0x00,0x00},/*"吃",4*/

    {0x00,0x20,0xF8,0x63,0x08,0x21,0xFF,0x1F,0x08,0x11,0xF8,0x19,0x20,0x30,0x18,0x00},
    {0xC8,0x3F,0x08,0x44,0x09,0x42,0x0E,0x41,0x88,0x40,0x28,0x40,0x18,0x78,0x00,0x00},/*"蛇",5*/

    {0x10,0x02,0x10,0x42,0x10,0x81,0xFF,0x7F,0x90,0x00,0x20,0x00,0x98,0x80,0x88,0x84},
    {0x88,0x4B,0xE9,0x28,0x8E,0x10,0x88,0x28,0x88,0x47,0xA8,0x80,0x98,0x00,0x00,0x00},/*"按",6*/

    {0x00,0x01,0x80,0x00,0x60,0x00,0xF8,0xFF,0x87,0x00,0x80,0x40,0x84,0x40,0x84,0x40},
    {0x84,0x40,0xFE,0x7F,0x82,0x40,0x83,0x40,0x82,0x40,0x80,0x40,0x80,0x00,0x00,0x00},/*"任",7*/

    {0x10,0x40,0x10,0x30,0x12,0x00,0xD2,0x77,0x56,0x85,0x5A,0x85,0x52,0x8D,0x53,0xB5},
    {0x52,0x85,0x5A,0x85,0x56,0x85,0xD2,0xE7,0x12,0x00,0x10,0x10,0x10,0x60,0x00,0x00},/*"意",8*/

    {0x40,0x01,0x30,0x01,0xEF,0x7F,0x24,0x21,0x24,0x51,0x80,0x26,0xE4,0x18,0x9C,0x27},
    {0x10,0x44,0x54,0x45,0x54,0x45,0xFF,0x5F,0x54,0x45,0x7C,0x45,0x10,0x44,0x00,0x00},/*"键",9*/

    {0x80,0x00,0x82,0x80,0x82,0x40,0x82,0x30,0xFE,0x0F,0x82,0x00,0x82,0x00,0x82,0x00},
    {0x82,0x00,0x82,0x00,0xFE,0xFF,0x82,0x00,0x82,0x00,0x82,0x00,0x80,0x00,0x00,0x00},/*"开",10*/

    {0x10,0x40,0x10,0x22,0xF0,0x15,0x1F,0x08,0x10,0x16,0xF0,0x21,0x00,0x00,0x40,0x00},
    {0xE0,0xFE,0x58,0x42,0x47,0x42,0x40,0x42,0x50,0x42,0x60,0xFE,0xC0,0x00,0x00,0x00},/*"始",11*/
    
    {0x10,0x04,0x60,0x04,0x02,0x7E,0x8C,0x81,0x00,0x40,0x08,0x30,0xF9,0x0F,0x4E,0x40},
    {0xC8,0x7F,0x20,0x00,0x58,0x44,0x4F,0x84,0x48,0x7F,0xC8,0x04,0x08,0x04,0x00,0x00},/*"游",12*/

    {0x00,0x00,0x08,0x20,0x48,0x10,0x88,0x0C,0x08,0x03,0xC8,0x04,0x38,0x18,0x40,0x80},
    {0x40,0x40,0x40,0x20,0xFF,0x17,0x20,0x18,0x22,0x26,0xAC,0x41,0x20,0xF0,0x00,0x00},/*"戏",13*/

    {0x20,0x22,0x30,0x67,0xAC,0x22,0x63,0x12,0x20,0x12,0x18,0x12,0x08,0x00,0x48,0xFE},
    {0x48,0x42,0x48,0x42,0x7F,0x42,0x48,0x42,0x48,0x42,0x48,0xFE,0x08,0x00,0x00,0x00},/*"结",14*/

    {0x04,0x40,0x04,0x40,0xE4,0x27,0x24,0x22,0x24,0x12,0x24,0x0A,0x24,0x06,0xFF,0xFF},
    {0x24,0x06,0x24,0x0A,0x24,0x12,0x24,0x22,0xE4,0x27,0x04,0x40,0x04,0x40,0x00,0x00},/*"束",15*/
